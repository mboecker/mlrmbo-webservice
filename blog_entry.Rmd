...
We already showed [mlrMBO](link-to-mlrmbo-blogpost), a framework for MBO in R using the [mlr](https://mlr.mlr-org.com/) package.
However it can be impractical to maintain the installation of R, mlrMBO and all of its dependencies on all machines where you want to conduct optimization.
...
This allows us to create client libraries in [every programming language which supports REST](link to a list of REST clients in a lot of languages) (for example [Python](https://www.python.org) or [Julia](https://julialang.org)), but still use our state-of-the-art MBO package on the back-end.
...
There are already services which provide the described functionality.
...
The key difference there it only supports one specific kind of model, whereas mlrMBO (and therefore our optimization service) supports many learners, such as Kriging, which enabbles you to select a more fitting model for your specific usecase.
